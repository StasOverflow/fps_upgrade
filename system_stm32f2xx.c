...
uint8_t  flash_ob_bor_on_start;
uint8_t  flash_ob_rewritten;


void core_option_bytes_init( void )
{
    flash_ob_bor_on_start = FLASH_OB_GetBOR();
    if( flash_ob_bor_on_start != OB_BOR_LEVEL3 )
    {
        flash_ob_rewritten = 1;

        FLASH_OB_Unlock();

        FLASH_OB_BORConfig(OB_BOR_LEVEL3);

        while( FLASH_OB_Launch() == FLASH_BUSY ){};

        FLASH_OB_Lock();
    }
    else
    {
        flash_ob_rewritten = 0;
    }
}
